...
use crate::{config::PAGE_SIZE, mm::{StepByOne, VirtAddr, MapArea, PhysPageNum, MapPermission, MapType, PTEFlags, PageTableEntry, VirtPageNum, frame_alloc}, timer::{get_time_ms, get_time_us}};
...
pub fn sys_set_priority(prio: isize) -> isize {
    ...
    if prio < 2 {
        ...
        let task = current_task().unwrap();
        let priority = &mut task.inner_exclusive_access().priority;
